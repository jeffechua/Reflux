...
namespace Reflux::UI {
	...
	ApplicationMain::ApplicationMain() {
		...
		std::unique_ptr<Editor::DesignEditorWindow> window = std::make_unique<Editor::DesignEditorWindow>(*design);
	...
	bool ApplicationMain::show() {
		...
		for (auto& entry : designs) {
			std::unique_ptr<Editor::DesignEditorWindow>& window = entry.second;
			if (!window->show()) {
